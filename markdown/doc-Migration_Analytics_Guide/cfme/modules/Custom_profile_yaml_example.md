You can modify the `custom_profile.yaml` template to create a customized
analysis profile for scanning your workloads.

**custom\_profile.yaml template.**
...
```
  ...
  - If you change the profile name from `default`, you must [create a
    control
  ...
  - You can add or modify targets for `item_type: file`.

  - Do not change `content: true` for this target.
