...
Installing ManageIQ on Red Hat Virtualization consists of the following
steps:
...
Uploading the ManageIQ appliance file to Red Hat Virtualization
requires:
...
2.  Select **Red Hat Virtualization** from the **--Choose your
    platform--** list.
...
2.  Click <span class="menuchoice">Storage \> Disks</span>.

3.  Select <span class="menuchoice">Upload \> Start</span>.
...
machine containing the ManageIQ image and a second disk for the
database:

1.  In <span class="menuchoice">Compute \> Virtual Machines</span>,
...
4.  Select the ManageIQ appliance you uploaded from the list of images
    and click **OK**.
    ...
    3.  Specify a **Size** in GB for the disk that allows sufficient
        ...
        factors affect the size of your database over time:
          ...
          - Storage Count: the number of individual storage elements as
            ...
            See "Database Requirements" in the *Deployment Planning
            Guide* for size considerations.
...
Your Red Hat Virtualization environment now contains a running ManageIQ
appliance.

Next, configure the database on the disk you created by following the
steps in [Configuring an Internal
Database](#configuring-an-internal-database).
