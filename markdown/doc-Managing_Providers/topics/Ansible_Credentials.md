Credentials are utilized by {product-title} for authentication when
...
{product-title} can store credentials used by playbooks. Credentials
saved in {product-title\_short} are matched and executed with a playbook
when run.

1.  Navigate to menu:Automation\[Ansible \> Credentials\].

2.  Click ![Configuration](1847.png) (**Configuration**), then ![Add a
    New Credential](1862.png) (**Add a New Credential**).
...
Each credential type used by {product-title\_short} is detailed in the
following sections.

## Machine

Machine credentials enable {product-title\_short} to invoke Ansible on
...
If both {product-title\_short} and a VMware provider are located in the
